# Run the DQS pipeline from the root directory using targets-only workflows.
...
cat("Running DQS targets-only pipeline with:\n")
...
writeLines(c(
  ...
  "  lapply(source)",
  "list(",
  sprintf("  tar_target(dqs_client_files, '%s'),", data_path),
  sprintf("  tar_target(dqs_db_file, '%s'),", output_catalogue),
  "  tar_target(dqs_pipeline, dqs(dqs_client_files) |> dqs_export(dqs_db_file))",
  ")"
), con = pipeline_file)
...
tryCatch({
  ...
  cat("DQS targets-only pipeline complete.\n")
}, finally = {
